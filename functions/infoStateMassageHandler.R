infoStateMassageHandler <- function(info_state, output, selected_dir = NULL) {
  switch(info_state,
         "initial" = {
         ...
         "loaded" = {
           ...
           msg <- paste0("Dataset '", basename(selected_dir), "' was successfully loaded.")
         ...
         "processed" = {
           ...
           msg <- paste0("Dataset '", basename(selected_dir), "' was successfully processed")
           showNotification(msg, duration = 5, type = "default")
         },
         "fitErrorRecal" = {
           output$info1 <- renderText(("Dataset:"))
           ...
           msg <- paste0("'", basename(selected_dir), "' could not be processed. Wrong lock-mass?")
         ...
         "fitErrorNorm" = {
           ...
           msg <- paste0("'", basename(selected_dir), "' could not be processed. Wrong normalization-mass?")
           showNotification(msg, duration = 15, type = "error")
         },
         "RefMzError" = {
           output$info1 <- renderText(("Dataset:"))
           ...
           msg <- paste0("'", basename(selected_dir), "' could not be processed. Increase tolerance?")
           showNotification(msg, duration = 15, type = "error")
         },
         "fitErrorOther" = {
           output$info1 <- renderText(("Dataset:"))
           ...
           msg <- paste0("'", basename(selected_dir), "' could not be processed. Unknown error")
           showNotification(msg, duration = 15, type = "error")
         },
         "loadErrorNum" = {
           ...
           msg <- paste0("Failed to load '", basename(selected_dir), "'. All folders need to have concentrations as names.")
           showNotification(msg, duration = 15, type = "error")
         },
         "loadErrorFormat" = {
           output$info1 <- renderText(("Failed to load:"))
           ...
           msg <- paste0("Failed to load '", basename(selected_dir), "'. Wrong file format.")
           showNotification(msg, duration = 15, type = "error")
         },
         "loadErrorUniqueMz" = {
           ...
           msg <- paste0("Failed to import '", basename(selected_dir), "'. Non-unique m/z found.")
           showNotification(msg, duration = 15, type = "error")
         },
         "loadErrorMapping" = {
           output$info1 <- renderText(("Failed to load:"))
           ...
           msg <- paste0("Failed to load '", basename(selected_dir), "'. Mapping file error.\nNumber conc. != Number spectra.")
           showNotification(msg, duration = 15, type = "error")
         },
         "errorMapping" = {
           output$info1 <- renderText(("Failed to load:"))
